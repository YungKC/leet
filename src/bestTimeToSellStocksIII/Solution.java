package bestTimeToSellStocksIII;
...
public class Solution {
	...
	public int maxProfit(int[] prices) {
        ...
        for (int i = 1; i <= k; i++) {
            ...
            int tmpMax =  state[i-1][0]-prices[0];
            for (int j = 1; j < length; j++) {
                state[i][j] = Math.max(state[i][j - 1], prices[j] + tmpMax);
                tmpMax =  Math.max(tmpMax, state[i - 1][j - 1] - prices[j]);
            }
        }
