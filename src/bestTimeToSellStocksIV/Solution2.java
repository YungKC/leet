...
public class Solution2 {
	...
	public int maxProfit(int k, int[] prices) {
    	...
    	searchBullCycleRecursive(prices, startIndex, endIndex);
		int result = 0;
	...
	private void searchBullCycleRecursive(int[] prices, int startIndex, int endIndex) {
    	...
    	while (startIndex < endIndex && prices[startIndex+1] <= prices[startIndex]) {
    		startIndex++;
    	}
    	while(endIndex > startIndex && prices[endIndex] <= prices[endIndex-1]) {
    		endIndex--;
    	}
		...
		// every drop in price within this range is a potential profit
		for (int i = lowestIndex+1; i<highestIndex; i++) {
			if (prices[i+1] < prices[i]) {
				int tmpHigh = prices[i];
				i++;
				while (i<highestIndex && prices[i+1] <= prices[i]) {
					i++;
				}
				acceptedProfitPotential.add(tmpHigh-prices[i]);
			}
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(sol.maxProfit(5, new int[]{1,2,1,1,2,1}) == 2);
		System.out.println(sol.maxProfit(5, new int[]{1,2,1,2,1}) == 2);
